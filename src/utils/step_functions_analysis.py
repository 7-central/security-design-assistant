...
class StepFunctionsAnalyzer:
    ...
    def _analyze_stage(
        self, stage_name: str, timings: list[float], success_data: dict[str, int]
    ...
    def generate_recommendations(self) -> list[WorkflowRecommendation]:
        ...
        high_risk_stages = [
            stage
            for stage in self.stage_analyses.values()
            ...
            "States": {
                "PDFProcessing": {
                    ...
                    "Resource": (
                        ...
                        "security-assistant-pdf-processor-${Environment}"
                    ),
                    "TimeoutSeconds": 300,
                    "Retry": [
                        {
                    ...
                    "Catch": [{"ErrorEquals": ["States.ALL"], "Next": "ProcessingFailed", "ResultPath": "$.error"}],
                    "Next": "CheckContextRequired",
                },
                "CheckContextRequired": {
                    "Type": "Choice",
                    "Choices": [
                        {"Variable": "$.context_s3_key", "IsPresent": True, "Next": "ContextProcessing"},
                        {"Variable": "$.context_text", "IsPresent": True, "Next": "ContextProcessing"},
                    ],
                ...
                "ContextProcessing": {
                    ...
                    "Resource": (
                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                        "security-assistant-context-processor-${Environment}"
                    ),
                    "TimeoutSeconds": 300,
                    "Retry": [
                        {
                    ...
                    "Catch": [
                        {"ErrorEquals": ["States.ALL"], "Next": "DrawingAnalysis", "ResultPath": "$.context_error"}
                    ],
                ...
                "DrawingAnalysis": {
                    ...
                    "Resource": (
                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                        "security-assistant-drawing-analyzer-${Environment}"
                    ),
                    "TimeoutSeconds": 900,
                    "Retry": [
                        {
                    ...
                    "Branches": [
                        {
                            ...
                            "States": {
                                "ExcelGeneration": {
                                    ...
                                    "Resource": (
                                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                                        "security-assistant-excel-generator-${Environment}"
                                    ),
                                    "TimeoutSeconds": 600,
                                    "End": True,
                                }
                        ...
                        },
                        {
                            ...
                            "States": {
                                "QualityEvaluation": {
                                    ...
                                    "Resource": (
                                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                                        "security-assistant-evaluator-${Environment}"
                                    ),
                                    "TimeoutSeconds": 400,
                                    "End": True,
                                }
                ...
                "ProcessingComplete": {
                    ...
                    "Resource": (
                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                        "security-assistant-finalizer-${Environment}"
                    ),
                    "TimeoutSeconds": 60,
                    "End": True,
                },
                "ProcessingFailed": {
                    ...
                    "Resource": (
                        "arn:aws:lambda:${AWS::Region}:${AWS::AccountId}:function:"
                        "security-assistant-failure-handler-${Environment}"
                    ),
                    "TimeoutSeconds": 60,
                    "End": True,
                },
    ...
    def generate_analysis_report(self, job_history: list[dict]) -> dict:
        ...
        avg_processing_time = (
            sum(job.get("total_processing_time_seconds", 0) for job in job_history) / total_jobs
            if total_jobs > 0
            else 0
        )
...
def run_step_functions_analysis(job_history_file: str) -> dict:
    ...
    try:
        ...
        with open(job_history_file) as f:
